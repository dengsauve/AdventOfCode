...
def rotate_row(screen, index, amount)
  screen[index].rotate!(amount - 1)
  return screen
end

def rotate_column(screen, index, amount)
  screen.transpose.each_with_index do |i, col|
    col.rotate!(amount - 1) if i == index
  end
  return screen
end

def illuminate(screen, dimensions)
  x, y = dimensions.split('x')[0].to_i, dimensions.split('x')[1].to_i
  print x, y
  ret_screen = screen
...
steps.each do |step|
  commands = step.split
  if commands[0] == 'rect'
    screen = illuminate(screen, commands[1])
  elsif commands[0] == 'rotate'
    if commands[1] == 'row'
      screen = rotate_row(screen, commands[2][2..-1].to_i, commands[4].to_i)
    elsif commands[1] == 'column'
      screen = rotate_column(screen, commands[2][2..-1].to_i, commands[4].to_i)
    end
  end
  puts '','','',''
  light_up(screen)
